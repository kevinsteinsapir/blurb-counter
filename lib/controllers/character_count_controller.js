...
export default class extends Controller {
  ...
  updateCounter(event) {
    ...
    if (inputLength <= 200 && inputLength >= 100) {
      ...
      this.counterTarget.innerText = `${inputLength} caracteres. Te falta. Mejor mandale un mail a la Cata para que te saque del programa.`;
      this.counterTarget.classList.remove('text-danger');
  ...
  updateCounter2(event) {
    ...
    if (wordCount <= 125 && wordCount >= 80) {
      ...
      this.counter2Target.innerText = `${wordCount} es tu conteo de palabras. Not Ready, Not Sexy.`;
      this.counter2Target.classList.remove('text-danger');
